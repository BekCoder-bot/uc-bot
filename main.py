...
PUBG_API_KEY = os.environ.get("PUBG_API_KEY")  # Renderda yoki .env da saqlanadi

# To‘lov kartalari
payment_cards = {
    "Visa": {
        "card_number": "4916 9902 0011 5456",
        "name": "Egamberdiyev Og'abek"
    },
    "Uzcard": {
        "card_number": "5614 68871022 5581",
        "name": "Egamberdiyev Og'abek"
    }
}
...
def start_handler(message):
    ...
    bot.send_message(chat_id, "🇺🇿 Iltimos, tilni tanlang:", reply_markup=markup)

# Til tanlanganda salomlashish va UC so‘rash
...
def language_selected(message):
    ...
    bot.send_message(chat_id, "Assalomu alaykum! UC buyurtma berish uchun qancha UC kerakligini tanlang 👇", reply_markup=uc_options_keyboard())

# UC variantlari tugmalari
def uc_options_keyboard():
    markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        "60 + 3 UC - $0.89", "600 + 90 UC - $8.99",
        "1500 + 375 UC - $22.49", "3000 + 1000 UC - $44.49",
        "6000 + 2400 UC - $89.99", "12000 + 4800 UC - $179.99",
        "18000 + 7200 UC - $269.99", "24000 + 9600 UC - $359.99",
        "30000 + 12000 UC - $449.99", "60000 + 24000 UC - $899.99"
    )
    return markup

# UC tanlanganda PUBG ID so‘rash
@bot.message_handler(func=lambda msg: "UC - $" in msg.text)
def uc_selected(message):
    chat_id = message.chat.id
    user_data[chat_id]["uc"] = message.text

    bot.send_message(chat_id, "🔢 Iltimos, PUBG ID'ingizni kiriting:")
    bot.register_next_step_handler(message, get_pubg_id)

# PUBG ID qabul qilinib, to‘lovni amalga oshirishga yo‘naltirish
...
# To‘lov qilindi tugmasi bosilganda adminga habar yuboriladi
...
def payment_done(message):
    ...
    admin_id = 1382850686  # Sizning admin ID
    ...
    msg = (
        ...
        f"💸 UC: {info['uc']}\n"
        f"💳 To‘lov kartasi: {payment_cards['Visa']['card_number']} (Visa), {payment_cards['Uzcard']['card_number']} (Uzcard)"
    )
    bot.send_message(admin_id, msg)
    bot.send_message(chat_id, "✅ Buyurtmangiz qabul qilindi. Tez orada bog‘lanamiz!")

# PUBG ID ni tekshiradi
